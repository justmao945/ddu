...
pub struct Config {
    ...
    /// on the stored chord *instead* of them (the shell applies this in
    /// `keys::apply_overrides`). Entries the app cannot use
...
pub struct SavedSession {
    ...
    /// `AppView::restore_sessions`; every save writes an
