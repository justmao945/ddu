...
impl AppView {
    ...
    fn preview_is_current(&self, build: &ddu_diff::file_view::PreviewBuild) -> bool {
    ...
    /// poll used to cause.
    pub(crate) fn cached_file_view(&self) -> Option<&ddu_diff::file_view::FileView> {
    ...
    pub(crate) fn preview_refusal(&self) -> Option<ddu_diff::read::Unreadable> {
        let build = self.preview.as_ref().filter(|b| self.preview_is_current(b))?;
    ...
    pub(crate) fn selected_file_text(&self) -> String {
        ...
        if let Some(ddu_diff::file_view::FileView::Text(view)) = self.cached_file_view() {
            return view
                .rows
                .iter()
                .filter_map(|row| {
        ...
        cx.spawn(async move |_, cx| {
            ...
            let built = cx
                .background_spawn(async move {
                    let view = want_view.then(|| {
                        ddu_diff::file_view::FileView::build(&job_root, &job_path, file.as_ref())
                    });
                    let source = want_preview
                        .then(|| ddu_diff::read::read_source(&job_root, &job_path));
                    (view, source)
                })
                .await;
            ...
            let _ = this.update(cx, |v, cx| {
                ...
                if let Some(source) = source {
                    ...
                    v.preview = Some(ddu_diff::file_view::PreviewBuild {
