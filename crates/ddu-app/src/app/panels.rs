...
mod panel_cache_tests {
    ...
    fn stream_wakeup_repaints_only_the_terminal_pane() {
        gpui::run_test_once(
            ...
            Box::new(|dispatcher| {
                ...
                let wake = |term: &Entity<ddu_terminal::TermSession>, cx: &mut gpui_kit::App| {
                    term.update(cx, |_, cx| cx.emit(ddu_terminal::TermEvent::Wakeup));
                };
    ...
    fn a_background_rows_title_follows_its_own_stream() {
        gpui::run_test_once(
            ...
            Box::new(|dispatcher| {
                ...
                cx.executor().advance_clock(ddu_terminal::STREAM_FRAME_MIN);
    ...
    fn select_path_pins_clean_and_changed_files_alike() {
        use ddu_diff::{DiffFile, GitDiff, Snapshot};
        gpui::run_test_once(
            0,
            Box::new(|dispatcher| {
    ...
    fn a_selection_in_the_changes_pane_survives_a_frame_it_did_not_ask_for() {
        ...
        use ddu_core::config::{Config, LoadWarnings, ShellConfig, State};
        use ddu_diff::{DiffFile, DiffHunk, DiffLine, GitDiff};
            ...
            Box::new(|dispatcher| {
                ...
                vcx.update(|window, cx| {
                    view.update(cx, |v, cx| {
                        ...
                        v.snapshot = Some(ddu_diff::Snapshot {
                            diff: GitDiff {
                                branch: None,
                                files,
                            },
