...
pub(crate) fn accel_hint(id: &str, cx: &App) -> String {
    let cfg = cx.try_global::<ddu_core::config::Config>();
...
pub fn validate_keys(cfg: &Config) -> Vec<String> {
    let mut warnings = Vec::new();
    let file = ddu_core::config::settings_path().display().to_string();
...
pub(crate) fn apply_overrides(cx: &mut App) {
    let cfg = cx
        .try_global::<ddu_core::config::Config>()
        .cloned()
        .unwrap_or_default();
