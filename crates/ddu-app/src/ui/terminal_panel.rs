...
//! grid via [`ddu_terminal::element::TerminalElement`].
            ...
            move |_, event: &KeyDownEvent, window, cx| {
                ...
                if let Some(term) = weak.upgrade() {
                    ...
                    term.update(cx, |s, cx| {
                        s.write(&bytes);
                        s.scroll_to_bottom();
                        cx.emit(ddu_terminal::TermEvent::Wakeup);
                    });
                    cx.stop_propagation();
                }
            }
        ...
        .context_menu({
            ...
            move |menu, _window, cx| {
                ...
                if weak.upgrade().is_some_and(|t| {
                    t.read(cx).mouse_tracking() != ddu_terminal::MouseTracking::None
                }) {
                    return menu;
                }
...
fn empty_state(this: &AppView, cx: &mut Context<AppView>) -> impl IntoElement {
    ...
    let error = this.current_session().and_then(|s| match &s.status {
        ddu_core::session::AgentStatus::Error(error) => Some(error.clone()),
        _ => None,
    });
