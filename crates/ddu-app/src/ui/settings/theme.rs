...
use ddu_core::config::{TERMINAL_FONT_SIZE_DEFAULT, TERMINAL_SCROLLBACK_DEFAULT};
...
pub(crate) fn set_theme(mode: ThemeMode, cx: &mut App) {
    ...
    // platform family).
    Theme::global_mut(cx).font_size = px(ddu_core::config::ui_font_size());
...
pub(crate) fn bump_font_size(delta: f32, cx: &mut App) {
    let current = cx.global::<ddu_core::config::Config>().terminal_font_size();
...
pub(super) fn terminal_size_item() -> SettingItem {
    super::item(
        ...
        SIZE_DESCRIPTION,
        |options, window, cx| {
            let size = cx.global::<ddu_core::config::Config>().terminal_font_size() as f64;
            number_spinner(
                "size",
                options,
                window,
    ...
    .on_reset(
        |cx| {
            cx.global::<ddu_core::config::Config>()
                .terminal_font_size
                .is_some()
        },
        |_, cx| {
            Theme::global_mut(cx).mono_font_size =
                px(TERMINAL_FONT_SIZE_DEFAULT * ddu_core::config::desktop_text_scale());
...
pub(super) fn terminal_scrollback_item() -> SettingItem {
    super::item(
        ...
        |options, window, cx| {
            let lines = cx.global::<ddu_core::config::Config>().terminal_scrollback() as f64;
            number_spinner(
                "scrollback",
                options,
                window,
