...
fn row(
    ...
    let cfg = cx.global::<ddu_core::config::Config>();
...
mod tests {
    ...
    use ddu_core::config::{Config, State};
    ...
    /// Serializes this module's env-var tests (`DDU_SETTINGS_PATH`).
    ///
    /// Per *process*, so this crate has its own: the suite runs one test
    /// binary per crate, and ddu-core's copy guards ddu-core's tests in a
    /// binary of its own.
