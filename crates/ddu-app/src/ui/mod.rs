...
pub(crate) fn scaled(base: f32) -> f32 {
    base * ddu_core::config::desktop_text_scale()
}
...
pub fn apply_mono_typography(cx: &mut App) {
    let config = cx.global::<ddu_core::config::Config>();
    ...
    let code = ddu_core::config::ui_font_size() * 0.93;
...
/// [`ddu_core::config::ui_font_size`], so the heading base must too — that
...
pub(crate) fn document_text_style() -> gpui_kit::component::text::TextViewStyle {
    gpui_kit::component::text::TextViewStyle {
        heading_base_font_size: px(ddu_core::config::ui_font_size()),
        ..Default::default()
    }
}
