...
pub enum FileView {
    ...
    Image,
    /// NUL byte in the first [`crate::read::BINARY_SNIFF_BYTES`].
    Binary,
    /// Over [`crate::read::MAX_VIEW_BYTES`] or [`crate::read::MAX_VIEW_LINES`].
    TooLarge,
...
mod tests {
    use super::*;
    use crate::read::{MAX_VIEW_BYTES, read_source};
    use crate::{DiffHunk, DiffLine};
    ...
    fn deep_rows_are_indexed_and_searchable() {
        ...
        let stream = crate::RowStream::view(&view);
        ...
        assert!(matches!(last, crate::PaneRow::Line(l) if l.text == "LINE 19999 CHANGED"));
        ...
        assert!(
            matches!(stream.row(hits[0]), Some(crate::PaneRow::Line(l)) if l.kind == '+')
        );
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn build_reads_merges_and_falls_back() {
        ...
        assert!(crate::RowStream::view(&clean).note().is_none());
