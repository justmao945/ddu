...
/// The `git2` this crate's query layer speaks. Re-exported so a caller
/// holding a repository handle (the shell discovers one per project and
/// passes it into [`git::snapshot`]) names the *same* libgit2 the diff
/// was built against, instead of declaring a second dependency that can
/// drift.
pub use git2;
...
enum RowSrc<'a> {
    Diff(&'a DiffFile),
    View(&'a crate::file_view::TextFileView),
}
...
impl<'a> RowStream<'a> {
    ...
    pub fn view(v: &'a crate::file_view::TextFileView) -> Self {
...
mod tests {
    ...
    fn view_stream_matches_are_row_indices() {
        ...
        let crate::file_view::FileView::Text(view) =
            crate::file_view::FileView::build(&dir, "f.txt", None)
        else {
            panic!("text view");
        };
