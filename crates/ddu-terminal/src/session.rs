...
impl TermSession {
    ...
    /// panel on the first frame. `scrollback` caps the grid's history
    /// (lines): it is the caller's user setting (`Settings → Terminal →
    /// Scrollback`), passed in rather than read from a global, so this
    /// crate owns no app config.
    pub fn spawn(cmd: &PtySpawn, scrollback: usize, cx: &mut App) -> anyhow::Result<Entity<Self>> {
        let (cols, rows) = (80, 24);
    ...
    pub fn capture_resume_id(&mut self) {
    ...
    pub fn element(weak: WeakEntity<Self>, focus: FocusHandle) -> impl IntoElement {
    ...
    fn render_mutations_emit_wakeup() {
        gpui::run_test_once(
            ...
            Box::new(|dispatcher| {
                ...
                let wakeups = log
                    .iter()
                    .filter(|e| **e == crate::TermEvent::Wakeup)
                    .count();
                assert!(
                    wakeups >= 3,
