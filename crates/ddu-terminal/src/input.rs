...
mod tests {
    ...
    fn printable_goes_through() {
        assert_eq!(
            encode(&ks("a", Some("a"), mods(false, false, false, false))).as_deref(),
            Some(b"a".as_slice())
        );
        assert_eq!(
            encode(&ks("a", Some("A"), mods(false, false, true, false))).as_deref(),
            Some(b"A".as_slice())
        );
        assert_eq!(
            encode(&ks("space", Some(" "), mods(false, false, false, false))).as_deref(),
            Some(b" ".as_slice())
        );
    }
    ...
    fn control_keys() {
        assert_eq!(
            encode(&ks("enter", None, mods(false, false, false, false))).as_deref(),
            Some(b"\r".as_slice())
        );
        assert_eq!(
            encode(&ks("backspace", None, mods(false, false, false, false))).as_deref(),
            Some(b"\x7f".as_slice())
        );
        assert_eq!(
            encode(&ks("tab", None, mods(false, false, true, false))).as_deref(),
            Some(b"\x1b[Z".as_slice())
        );
        assert_eq!(
            encode(&ks("escape", None, mods(false, false, false, false))).as_deref(),
            Some(b"\x1b".as_slice())
        );
    }
    ...
    fn arrows_and_navigation() {
        assert_eq!(
            encode(&ks("up", None, mods(false, false, false, false))).as_deref(),
            Some(b"\x1b[A".as_slice())
        );
        assert_eq!(
            encode(&ks("left", None, mods(true, false, false, false))).as_deref(),
            Some(b"\x1b[1;5D".as_slice())
        );
        assert_eq!(
            encode(&ks("delete", None, mods(false, false, false, false))).as_deref(),
            Some(b"\x1b[3~".as_slice())
        );
        assert_eq!(
            encode(&ks("pageup", None, mods(false, false, false, false))).as_deref(),
    ...
    fn control_combos() {
        ...
        assert_eq!(
            encode(&ks("c", Some("c"), mods(true, false, false, false))).as_deref(),
            Some(&[0x03][..])
        );
        assert_eq!(
            encode(&ks("d", None, mods(true, false, false, false))).as_deref(),
            Some(&[0x04][..])
        );
        assert_eq!(
            encode(&ks("space", Some(" "), mods(true, false, false, false))).as_deref(),
            Some(&[0x00][..])
        );
    }
    ...
    fn alt_prefixes_escape() {
        assert_eq!(
            encode(&ks("x", Some("x"), mods(false, true, false, false))).as_deref(),
            Some(b"\x1bx".as_slice())
        );
    }
    ...
    fn platform_modifier_is_reserved() {
        assert_eq!(
            encode(&ks("t", Some("t"), mods(false, false, false, true))),
            None
        );
    }
}
