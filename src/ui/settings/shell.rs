...
pub(super) fn shell_program_item() -> SettingItem {
    super::item(
        "Program",
        "Login shell launched for each new Terminal session.",
        |options, _, cx| {
            ...
            Button::new("shell-program-select")
                ...
                .dropdown_menu_with_anchor(Anchor::TopLeft, move |menu, _, _| {
                    ...
                    shells.sort();
                    shells.dedup();
                    let mut m = menu.min_w(px(super::CONTROL_W)).check_side(Side::Right);
                    for shell in shells {
...
pub(super) fn default_session_item() -> SettingItem {
    super::item(
        "Default type",
        "Session type created by the sidebar and the new-session button.",
        |options, _, cx| {
            let cfg = cx.global::<crate::config::Config>().clone();
