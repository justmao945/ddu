...
* Pane modes: the hunk area (`ui/diff_panel.rs`) shows the selected file in one of two modes, toggled by `⌘⇧M` or the header's far-right icon button (per session, persisted). **File is the default**: a file is read, not skimmed, and the surface carries the code around the change.
